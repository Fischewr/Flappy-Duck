...
from objects.ducky import Ducky
...
def create_sprites():
    ...
    return Duck(sprites), Ducky(sprites), GameStartMessage(sprites), Score(sprites)


duck, ducky, game_start_message, score = create_sprites()
...
while running:
    ...
    if ducky.check_collision(sprites) and not gameover:
        gameover = True
        gamestarted = False
        GameOverMessage(sprites)
        pygame.time.set_timer(column_create_event, 0)
